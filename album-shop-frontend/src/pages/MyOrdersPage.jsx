...
export default function MyOrdersPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [orderData, albumRes] = await Promise.all([
          fetchMyOrders(token),
  ...
  return (
    <div className="w-full min-h-screen flex flex-col bg-gradient-to-br from-indigo-50 to-white">
      ...
      {/* Orders */}
      <main className="w-full flex-grow px-6 py-10 max-w-4xl mx-auto">
        <h2 className="text-3xl font-bold text-indigo-800 mb-6">My Orders</h2>
        {orders.length === 0 ? (
          ...
          <ul className="space-y-6">
            {orders.map((order) => {
              ...
              return (
                <li key={order.id} className="bg-white border p-4 rounded-xl shadow flex gap-4">
                  {/* Album Image */}
                  {album.image_url ? (
                    <img
                      src={album.image_url}
